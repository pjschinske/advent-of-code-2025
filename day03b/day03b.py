...
def findBiggest(bank, start, end):
    biggest = -1
    biggestIndex = -1
...
for bank in input:
    ...
    numOfCells = len(bank) - 1
    searchStart = 0
    bankBiggest = 0
    for i in range(12, 0, -1):
        biggest, biggestIndex = findBiggest(bank, searchStart, numOfCells - i + 1)
        searchStart = biggestIndex + 1
        bankBiggest += biggest * (10 ** (i - 1))
    print(bankBiggest)
    sum += bankBiggest
print(sum)
